...
class Trainer(object):
    ...
    def _train_epoch(self, train_data, epoch_idx):
        ...
        for batch_idx, data in enumerate(iter_data):
            ...
            self.optimizer.step()
            if getattr(self.args, "dead_code_reset", False):
                interval = max(1, int(getattr(self.args, "dead_code_reset_interval", 10)))
                if (batch_idx + 1) % interval == 0:
                    with torch.no_grad():
                        encoded = self.model.encoder(data)
                        reset_n = self.model.rq.reset_dead_codes(
                            encoded, indices,
                            threshold=int(getattr(self.args, "dead_code_threshold", 1)),
                        )
                    if reset_n:
                        self.logger.info("dead-code reset: %d codes", reset_n)
            self.scheduler.step()
